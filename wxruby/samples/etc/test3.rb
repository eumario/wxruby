...
class AboutDialog < Dialog
  def initialize(parent)
    super(parent, -1, "Enter a new password", DEFAULT_POSITION,
               Size.new(200, 200), DEFAULT_DIALOG_STYLE | RESIZE_BORDER)
    dialogSizer = BoxSizer.new(VERTICAL)

    controlSizer = FlexGridSizer.new(2, 2, 10, 10)

    controlSizer.add(StaticText.new(self, -1, "Username"), 0, ALIGN_CENTRE_VERTICAL)
    controlSizer.add(TextCtrl.new(self, -1), 0, ALIGN_CENTRE_VERTICAL)
    controlSizer.add(StaticText.new(self, -1, "Password"), 0, ALIGN_CENTRE_VERTICAL)
    controlSizer.add(TextCtrl.new(self, -1), 0, ALIGN_CENTRE_VERTICAL)
    controlSizer.add(StaticText.new(self, -1, "Retype Password"), 0, ALIGN_CENTRE_VERTICAL)
    controlSizer.add(TextCtrl.new(self, -1), 0, ALIGN_CENTRE_VERTICAL)

    buttonSizer = BoxSizer.new(VERTICAL)
    buttonSizer.add(Button.new(self, OK, "Ok"), 0, NORTH, 0)

    dialogSizer.add(controlSizer)
    dialogSizer.add(20,20)
    dialogSizer.add(buttonSizer, 0, ALIGN_CENTRE)

    set_sizer(dialogSizer)
    set_auto_layout(TRUE)
    layout()
	
	evt_button(OK) {end_modal(0)}
  end
...
#	@m_pTextCtrl = NumTextCtrl.new(self, -1, "Type some text...",DEFAULT_POSITION, DEFAULT_SIZE, TE_MULTILINE)
  ...
  def onMenuFileOpen(event)
	  ...
	  if dlg.show_modal() == ID_OK
	    @m_pTextCtrl.LoadFile(dlg.get_filename())
  ...
  def onMenuFileSave(event)
	  ...
	  if dlg.show_modal == ID_OK
	    @m_pTextCtrl.save_file(dlg.GetPath)
	    set_status_text(dlg.get_filename, 0)
	  end
	  dlg.destroy
  end
