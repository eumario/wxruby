...
class MyFrame < Frame
    ...
    def onChooseColour(event)
        ...
        dialog.set_title("Choose the background colour")
    ...
    def onChooseFont(event)
        data = FontData.new
        data.set_initial_font(get_app().m_canvasFont)
        data.set_colour(get_app().m_canvasTextColour)
    ...
    def onShowTip(event)
        ...
        tipProvider = create_file_tip_provider("tips.txt", @s_index)
...
class MyApp < App
    ...
    def on_init()
      ...
      frame = MyFrame.new(nil, "Windows dialogs example", Point.new(20, 20), Size.new(400, 300))
